...
class LocationAdmin(admin.ModelAdmin):
    ...
    def get_urls(self):
        ...
        my_urls = [
            path('<path:object_id>/massadd/', MassAddLocationsAdminView.as_view(), name='%s_%s_massadd' % info),
            path('massadd/', MassAddLocationsAdminView.as_view(), name='%s_%s_massadd' % info),
            path('<path:object_id>/print-label/', self.send_to_printer_view, name='%s_%s_print-label' % info),
        ]
        return my_urls + urls
    ...
    def mass_add_view(self, request, object_id=None):
        class MassAddForm(forms.Form):
            ...
            sub_count = forms.IntegerField(
                label='Sub-location count'
            )
        ...
        return TemplateResponse(request, "admin/inventory/location_massadd.html", context)
...
class MassAddLocationsForm(forms.Form):
    ...
    sub_count = forms.IntegerField(
        label='Sub-location count',
        required=False)
    sub_print = forms.BooleanField(
        label='print sub-location labels',
        required=False)
    ...
    def clean_sub_type(self):
        ...
        if sub_type and sub_type.unique and self.cleaned_data['sub_count']:
            raise ValidationError("Sub-Type must be a non-unique type, if sub-location count is >0.")
        return sub_type
...
class MassAddLocationsAdminView(FormView):
    ...
    def form_valid(self, form):
        ...
        # create locations
        for name, short_name in loc_type.generate_names(data['count'], start=data['sequence_start']):
            l = models.Location()
            l.name = name
